...
/**
 * A composable function that defines the layout and behavior of the Appointments screen within the application.
 ...
 * the title resource ID [R.string.title_schedule]. The content area is defined by a [Column] layout, which
 * can be populated with additional components as needed for displaying appointment details.
 *
...
fun AppointmentsScreen(
    ...
    Scaffold(
        topBar = {AppBar(title = R.string.title_schedule)}
    )
    { innerPadding ->
        Column(
            modifier = modifier
